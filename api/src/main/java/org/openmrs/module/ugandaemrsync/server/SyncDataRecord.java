...
public class SyncDataRecord {
	...
	public int syncRecords(List<SyncRecord> newSyncRecords) {
		/**
		...
		if (connectionStatus == SyncConstant.CONNECTION_SUCCESS) {
			...
			System.out.println(newSyncRecords.size());
			int size = 0;
			for (SyncRecord syncRecord : newSyncRecords) {
				try {
					Map response = syncData(syncRecord.getItems().toString());
					...
					if (success.equalsIgnoreCase("successful")) {
						...
						ugandaEMRRecord.updateSyncedRecord(syncRecord);
						size += 1;
					}
				}
				catch (Exception e) {
